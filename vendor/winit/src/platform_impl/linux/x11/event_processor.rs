...
impl EventProcessor {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        if xev.message_type == atoms[XdndPosition] as c_ulong {
            ...
            if self.dnd.result.is_none() && !self.dnd.selection_requested {
        ...
        if xev.message_type == atoms[XdndDrop] as c_ulong {
            if self.dnd.source_window.is_none() {
                let source_window = xev.data.get_long(0) as xproto::Window;
                unsafe {
                    self.dnd
                        .send_finished(window, source_window, DndState::Rejected)
                        .expect("Failed to reject XDND drop.");
                }
                self.dnd.reset();
                return;
            }
            if self.dnd.result.is_none() {
                // The source has already announced the drop, but its large URI
                // list may still be awaiting SelectionNotify or arriving through
                // ICCCM INCR chunks. Defer DroppedFile and XdndFinished.
                self.dnd.drop_requested = true;
                return;
            }
            self.dispatch_dnd_result(window, true, &mut callback);
            return;
        }
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        match unsafe { self.dnd.read_data(window) } {
            Ok(mut data) => {
                let parse_result = self.dnd.parse_data(&mut data);
                self.dnd.result = Some(parse_result);
                self.dispatch_dnd_result(window, self.dnd.drop_requested, &mut callback);
            }
            Err(error) if error.is_actual_property_type(atoms[INCR]) => unsafe {
                self.dnd.begin_incremental(window);
            },
            Err(_) => {
                self.dnd.result = Some(Err(
                    crate::platform_impl::x11::dnd::DndDataParseError::EmptyData,
                ));
                if self.dnd.drop_requested {
                    self.dispatch_dnd_result(window, true, &mut callback);
                }
            }
        }
    ...
    fn dispatch_dnd_result<T: 'static, F>(
        &mut self,
    ...
    {
        let window_id = mkwid(window);
        let state = if let Some(Ok(ref paths)) = self.dnd.result {
            for path in paths {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        if atom == atoms[XdndSelection]
            && xev.state == xlib::PropertyNewValue
            && self.dnd.incremental.is_some()
        {
            let window = xev.window as xproto::Window;
            let completed = unsafe { self.dnd.read_incremental_chunk(window) };
            match completed {
                Ok(Some(result)) => {
                    self.dnd.result = Some(result);
                    self.dispatch_dnd_result(window, self.dnd.drop_requested, &mut callback);
                }
                Ok(None) => {}
                Err(_) => {
